...
interface Props {
  ...
  disabled?: boolean
  getBadge?: (tribunal: TribunalType) => string | null
}

export function TribunalSelectorGrid({ tribunals, selected, onSelect, disabled, getBadge }: Props) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
      {tribunals.map((t) => {
        const isSelected = t.id === selected
        const badge = getBadge?.(t) ?? null
        return (
          <button
            key={t.id}
            type="button"
            ...
            disabled={disabled}
            className={[
              'relative rounded-xl border p-3 text-left transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed',
              isSelected
          ...
          >
            {badge && (
              <span className="absolute left-1 top-1 text-[6px] uppercase tracking-wider text-[#6b7280] bg-[#0c0c14]/80 border border-[#2a2a3e] rounded px-1.5 py-0.5 leading-none">
                {badge}
              </span>
            )}
